...
def get_modulus_from_net(name):
    return FSS.GetModulus(name)

def get_modulus_from_hsm(name):
    return GFI.get_create_public_rsa_modulus(name)
...
def create(output, keys, hsm, net):
    ...
    accessor = get_modulus_from_net if net else get_modulus_from_hsm
    ...
    with open(output, "wb") as f:
        ...
        for i, key in enumerate(keys):
            write_modulus(f, header_size, i, accessor(key))
...
def main():
    ...
    return create(args.file, keys=args.key_names, hsm=args.have_hsm, net=args.have_net)
