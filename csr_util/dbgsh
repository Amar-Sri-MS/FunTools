...
class csr_metadata:
    def __init__(self):
        loc = os.path.join(os.getcwd(), "csr_metadata.json")
...
class dbgsh(cmd2.Cmd):
    ...
    parser_peek = subparsers.add_parser('peek', help="help peek",
    ...
    parser_poke = subparsers.add_parser('poke', help="help poke",
                                        formatter_class=ArgFormatter)
    ...
    parser_find.add_argument('csr', nargs=1, type=str, help="substring of csr name")
    ...
    parser = argparse.ArgumentParser(prog='i2c', formatter_class=ArgFormatter)
    subparsers = parser.add_subparsers(help='i2c sub-command help')
    parser_find = subparsers.add_parser('connect',
                help="connect to i2c server", formatter_class=ArgFormatter)
    parser_find.set_defaults(func=server_connect)
    parser_find.add_argument('ip_addr', nargs=1, type=str,
                help="ip address of i2c server")

    parser_find = subparsers.add_parser('disconnect',
                help="disconnect from i2c server", formatter_class=ArgFormatter)
    parser_find.set_defaults(func=server_disconnect)
    argcomplete.autocomplete(parser)
    @with_argparser(parser)
    def do_i2c(self, args):
        'i2c server connect/disconnect'
        logger.debug(args)
        args.func(args)
