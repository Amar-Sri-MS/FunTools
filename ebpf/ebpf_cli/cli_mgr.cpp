...
void
cli_mgr::__init(const char *host, const uint16_t & port)
{
                      ...
                      std::unordered_map < std::string, std::string > &params)
{
    ...
    /*
     * Remove from this string all newline chars
     */

    snd_jstr.erase(std::remove(snd_jstr.begin(),
			    snd_jstr.end(),
			    '\n'), snd_jstr.end());
    ...
    /* Send whatever you get to FunOS waiting.
    * The module must send in a valid JSON */
