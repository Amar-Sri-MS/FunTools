...
tcp_cli::tcp_cli(void):sock(-1), net_buf
{
new char[NBUF_SZ] ()} {
}
...
bool tcp_cli::send_js_data(void* json_root) {
    auto json = (struct fun_json *)(json_root);
    auto rval = fun_json_write_to_fd(json, sock);
    if (!rval) {
        perror("Send failed");
	return false;
    ...
    auto output = fun_json_read_from_fd(sock);
    char* pp = fun_json_to_text(output);
    std::cout << "RX:" << pp << std::endl;
