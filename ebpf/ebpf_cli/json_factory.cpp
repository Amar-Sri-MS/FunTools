 ...
 *  Implements factory creational mechanism for generation of
 *  a valid JSON. Also creates an adapter layer on top of our
 *  pure C JSON infrastructure for easy interface with dpc
 *
...
void *json_factory::create_arr(const std::vector<std::string> &vec) {
    ...
    for (auto &val : vec) {
        fun_json_array_append(
            json_root, fun_json_create_string(val.c_str(), fun_json_copy));
    }
    return (void *)json_root;
}
...
bool json_factory::add_bin_to_dict(void *root, const std::string &key,
    ...
    return fun_json_dict_add((struct fun_json *)root, key.c_str(),
                             fun_json_copy, bin_json, true);
}
...
bool json_factory::add_json_to_dict(void *&root, const std::string &key,
                                    void *&val) {
    return fun_json_dict_add((struct fun_json *)root, key.c_str(),
                             fun_json_copy, (struct fun_json *)val, true);
}
...
std::string json_factory::stringify(void *json_root) {
    ...
    /*
     * Remove from this string all newline chars for network transmission
     */
