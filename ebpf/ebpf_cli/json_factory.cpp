...
void *json_factory::create_arr(const std::vector < std::string > &vec)
{
	...
	for (auto & val:vec) {
		fun_json_array_append(json_root,
				      fun_json_create_string(val.c_str(),
							     fun_json_copy));
	}
	return (void *)json_root;
}
...
void *json_factory::start(const std::string & verb, int64_t tid)
{
	...
	fun_json_dict_add_int64(json_root,
				"tid", fun_json_copy, tid, true);

	return (void *)(json_root);
}

void *json_factory::create_dict(const std::unordered_map < std::string,
				std::string > &mp)
{
	struct fun_json *json_root = fun_json_create_empty_dict();
				   ...
				   uint8_t * val, const uint16_t sz)
{
	...
	return fun_json_dict_add((struct fun_json *)root,
				 key.c_str(), fun_json_copy, bin_json, true);
...
{
	return fun_json_dict_add_string((struct fun_json *)root,
					key.c_str(),
					fun_json_copy,
					val.c_str(), fun_json_copy, true);
...
std::pair<uint8_t*, size_t> json_factory::get_binary(void* json_root) {
   size_t alloc_sz;
   auto [ptr, size] = fun_json_serialize((struct fun_json *)json_root, &alloc_sz);
