...
*/ std::unordered_map <
std::string, std::string > tokenize(const std::string & buf)
{
...
void help(void)
{
	...
	std::cout << std::endl;
	std::cout <<
	    "**** BPF shell frontend for transfering/installing ebpf bytes to FunOS ***"
	    << std::endl;
	...
	std::cout <<
	    "order: <order_num> (attachment ordering for this bpf code, lower number gets executed first)"
	    << std::endl;
...
int main(int argc, char **argv)
{
	...
	while ((opt_char =
		getopt_long(argc, argv, "hS:p:", long_opts, NULL)) != -1) {
		switch (opt_char) {
		case 'h':
			help();
			exit(0);
