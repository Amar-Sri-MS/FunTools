...
void help(void)
{
	...
	std::cout << std::endl;
	std::cout <<
	    "**** Commandline frontend for transfering/installing ebpf bytes to FunOS ***"
	    << std::endl;
	std::cout << "-h: This help" << std::endl;
	std::cout <<
	    "-S {hostname} | --dpc_host={hostname}: remote dpc host to connect to (default: localhost)"
	    << std::endl;
	std::cout <<
	    "-p {portnum}  | --dpc_port={portnum}: dpc host port to connect to (default:"
	    << DPC_PORT << ")" << std::endl;

	std::cout <<
	    "-m {modulename}  | --module_name={module}: module inside FunOS to target(default:"
	    << DEF_MOD << ")" << std::endl;
	std::cout << std::endl;
	std::cout << std::endl;
	...
	std::
	    cout << "sec=(xdp) (FunOS section to attach to. [Attach & Detach]"
	    << std::endl;
	std::
	    cout << "cl_id=<client_id> (unique client id    [Attach & Detach)"
	    << std::endl;
	std::
	    cout <<
	...
	std::cout << "bpf=(location to bpf object byte code [Attach only]" <<
	    std::endl;
	std::cout << "qid: <QID> (XDP, QID to attach to [Attach only])" << std::
	    endl;
	std::
	    cout <<
	    "order: <order_num> (Relative order of execution of this bpf code, lower number gets executed first [Attach only])"
	    << std::endl;
...
int main(int argc, char **argv)
{
	...
	while ((opt_char =
		getopt_long(argc, argv, "hm:S:p:i", long_opts, NULL)) != -1) {
		switch (opt_char) {
		case 'h':
			help();
			exit(0);
