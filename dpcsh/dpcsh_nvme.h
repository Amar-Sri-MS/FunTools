...
extern bool _write_to_nvme(struct fun_ptr_and_size data, struct dpcsock *sock,
		    uint32_t sess_id, uint32_t seq_num);
extern uint32_t _read_from_nvme(uint8_t **data, uint8_t **deallocate_ptr, struct dpcsock *sock, uint32_t sess_id,
				 uint32_t seq_num);
