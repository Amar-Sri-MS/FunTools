...
class EntrySubNode(object):
	...
	def read_api(self, csr_root, boffset):
		...
		for i in range(max_depth):
			...
			l_str = string.Formatter().vformat(indent(idx+1) + 'for (unsigned int i_{i} = {rmin};'
					+ ' i_{i} < {rmax}; i_{i}++) {ob}' + '\n' + '{r}' + '\n' + indent(idx+1) + '{cb}' , (),
					SafeDict(i=idx+1, rmin=rmin, rmax=rmax, r=r_str))

                        jstr = string.Formatter().vformat(jstr, (),  SafeDict(j=idx+1))
			j_str = string.Formatter().vformat((indent(idx+1) + "struct fun_json *dict{j} = fun_json_create_empty_dict();\n"
					...
					+ indent(idx+1) + '{cb}'), (), SafeDict(i=idx+1, rmin=rmin, rmax=rmax, r=jstr, j=idx))
...
class EntryNode(object):
	...
	def read_api(self, csr_root):
		...
		field_dict = ("const char *keys[] = {ob}" + field_dict + "{cb};")

		j_str = ("\n" + "static struct fun_json *{e_name}_{node}_create_json() {ob}\n"+
				indent(1) + field_dict + "\n" +
				indent(1) + "printf(\"****NAG calling %s%d\\n\", __func__, __LINE__);" + "\n" +
				indent(1) + j_str + "\n" +
                                indent(1) + "return dict0;\n" + "{cb}\n")
		j_str = string.Formatter().vformat(j_str, (),
				SafeDict(boffset=boffset, node=self.name, bsize=rbuf_size))
...
class ModuleRoot(object):
	def __init__(self, mod, data, csr_root):
	...
	def get_an_list(self):
        ...
        def get_static_include_paths(self, sdk_inc_dir):
             str = ""
             str += '#include <' + sdk_inc_dir + '/' + 'csr_acc.h>\n'
             str += '#include <utils/threaded/fun_json.h>\n'
             str += '#include <props_bridges/props_bridges.h>\n'
             str += '#include <utils/threaded/fun_props.h>\n'
             str += '#include <utils/threaded/fun_malloc_threaded.h>\n'
             return str
	...
	def props_bridge_paths(self):
		...
		for k,v in self.element_map.iteritems():
			...
			prop_elem_api += ("static struct fun_json *" + gen_api +
                                "(fun_props_bridge_context_t context)" + "{\n"+
				indent(1) + "struct fun_json *dict = fun_json_create_empty_dict();\n" +
                                v.get_props_bridge_api() +
                                indent(1) + "return dict;\n" + "}\n")
...
def main():
	...
	cfg = CFG_Reader(src_dir)
        sdk_csr_cfg_dir = sdk_dir + "/FunSDK/config/"
	csr_slrp = csr.Slurper(os.getcwd(), sdk_csr_cfg_dir)
	csr_root = csr_slrp.get_csr_defs()

	#Generate the funos source code
	inc_path = ""
	code_str = ""
	for i, (k,v) in enumerate(cfg.get().iteritems()):
		mod = ModuleRoot(k, v, csr_root)
                sdk_inc_dir = sdk_dir + "/FunSDK/chip/f1/hw/include/core"
                inc_path += mod.get_static_include_paths(sdk_inc_dir)
                sdk_inc_dir = sdk_dir + "/FunSDK/chip/f1/hw/include/csr"
		inc_path += mod.get_an_include_paths(sdk_inc_dir) + "\n"
