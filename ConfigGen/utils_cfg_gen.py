...
class HUCodeGen():
    ...
    This class generates c struct from JSON configuration
    ...
    def _gets_struct_def_field(self, k, v, obj_key, struct_def, str_field, int_field, enum_field, bool_field, int_array_field,
                               enum, enum_def_str):
    ...
    def _parse_schema(self, obj_key, obj_properties, maxItem, struct_def):
        ...
        for k, v in obj_properties.items():
            ...
            self._gets_struct_def_field(k, v, obj_key, struct_def, str_field, int_field,
                                        enum_field, bool_field, int_array_field, enum, enum_def_str)
    ...
    def _generate_header(self, cfg_name, key_to_gen, output_dir):
        ...
        schema_properties = self.schema_dict["properties"]

        print "Generating config data header file: ", cfg_name
        file_tmpl = Template('// Source file generated by ' + __file__ + ' at $date \n'\
        ...
        for k, v in schema_properties.items():
            ...
            if obj_type == 'object':
                ...
                print "ERROR: unsupported type", obj_type
                assert False
            ...
            if 'dim' in struct_def.keys():
                print k, " obj dim len %d" % struct_def['dim']
            else:
                print k, " NO dim len"
                assert False
    ...
    def _parse_args(self, _args, parse_dict, struct_def):
       ...
       for i, l in enumerate(_args):
            _arg = l
            print "_args ", _args, " type ", type(_args)
            if isinstance(_arg, basestring):
                if _arg != "all":
                    print "only allowed _arg is [all], but found: ", _arg
                    assert False
    ...
    def _gets_struct_array(self, struct_key, struct_val):
        ...
        print "Unknown struct_key: ", struct_key
        assert False
    ...
    def _generate_c(self, cfg_name, key_to_gen, output_dir):
        ...
        print "Generating config data C file for ", cfg_name

        schema_properties = self.schema_dict["properties"]
        file_templ = Template('// Source file generated by ' + __file__ + ' at $date \n'\
                                '// DO NOT change this file\n\n'\
                                '#include <build/posix/generated/$fname.h>\n\n' '$data\n')

        # struct head definition
        struct_head_tmpl = Template("\nstruct $sname $name$trail")

        struct_head_str, struct_objs_str = "", ""

        # dict to hold struct array
        struct_array_inst = {}

        header_file = cfg_name + ".c"
        f = open(os.path.join(output_dir, header_file), 'w')
        ...
        f.write(file_templ.substitute(data=struct_objs_str, fname=cfg_name,
                    date=datetime.datetime.now().strftime("%Y-%m-%d %H:%M")))
        f.close()

    def generate(self, cfg_name, key_to_gen, output_dir):
        """ Generate c header and source

        Args:
            cfg_name (string): string name for config object used for comment and file name
            key_to_gen (list): json configuraiton keys for generating C code
            output_dir (string): dir for output .h .c files
        ...
        self._generate_header(cfg_name, key_to_gen, output_dir)
        self._generate_c(cfg_name, key_to_gen, output_dir)
