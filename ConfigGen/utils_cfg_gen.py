...
class HUCodeGen():
    ...
    def _gets_struct_def_field(self, k, v, obj_key, struct_def, int_field, enum_field, bool_field, int_array_field,
        ...
        if v_type == "integer":
            ...
            logging.info("unknown type %s", v_type)
            assert False
    ...
    def _generate_header(self, cfg_name, key_to_gen, output_dir):
        ...
        logging.info("Generating config data header file: %s", cfg_name)
        ...
        for k, v in schema_properties.items():
            ...
            if obj_type == 'object':
                ...
                logging.error("ERROR: unsupported type %s", obj_type)
                assert False
    ...
    def _gets_entry_val(self, k, v, struct_def):
        ...
        if v_type == "int":
            ...
            logging.error("no matching : %s", v_type)
            assert False
                        ...
                        for vf in range(vf_start, vf_end):
                            ...
                            for k, v in l.items():
                                ...
                                if k == "hw_instance":
                                    ...
                                    logging.info("(%s:%d:%d:%d) hw_instance %s", huid, ctrl, pf, vf, v)
                                """
    ...
    def _gets_struct_array(self, struct_key, struct_val):
        ...
        logging.error("Unknown struct_key: %s", struct_key)
        assert False
    ...
    def _generate_c(self, cfg_name, key_to_gen, output_dir):
        ...
        logging.info("Generating config data C file for %s", cfg_name)
