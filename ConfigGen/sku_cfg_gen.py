...
class SKUCfgGen():
    ...
    def generate_eeprom(self):
        ...
        sdk_pcicfg_path = os.path.join('feature_sets', 'pcicfg')
        pci_cfg_dir = os.path.abspath(os.path.join(self.input_dir, '..', '..', sdk_pcicfg_path))
        if not os.path.isdir(pci_cfg_dir):
            wks = os.environ.get('WORKSPACE')
            if wks:
                pci_cfg_dir = os.path.abspath(os.path.join(wks, 'FunSDK', sdk_pcicfg_path))
                if not os.path.isdir(pci_cfg_dir):
                    pci_cfg_dir = None
