...
extension NSControl: SimulationParameterControl { }

extension NSBox {
    func addParameterSectionName(_ parameters: SimulationParameters) -> CGFloat {
        let sectionName = parameters.name
        let sectionFrame = NSRect(x: 2, y: 0.0, width: frame.width, height: 22)
        let sectionLabel = NSText(nonSelectableText: sectionName, frame: sectionFrame)
        sectionLabel.font = NSFont.boldSystemFont(ofSize: 13.0)
        addSubview(sectionLabel)
        return 6.0 - sectionLabel.bounds.size.height
    }
    func addParameterSection(_ parameters: SimulationParameters, yStart: CGFloat, keys: [SimulationParameterKey], keyView: inout NSView!) -> [SimulationParameterKey: SimulationParameterControl] {
        var y: CGFloat = yStart
        var table: [SimulationParameterKey: SimulationParameterControl] = [:]
        var flowEnabled = true // could be used to disable controls
        for key in keys {
            let category = parameters.keyCategory(key)
            let defaultValue = parameters.all[key]!
            if category == 0 && key.findString("->") != nil {
                let height: CGFloat = 40.0
                y -= 22.0
                flowEnabled = defaultValue == JSON.bool(true)
                let button = NSButton()
                button.setButtonType(.`switch`)
                button.title = parameters.keysToUILabels[key]!
                button.imagePosition = .imageLeft
                button.state = flowEnabled ? 1 : 0
                button.alignment = NSTextAlignment.left
                button.frame = NSRect(x: 10, y: y, width: frame.width, height: 22)
                button.toolTip = parameters.toolTips[key]
                button.font = NSFont.systemFont(ofSize: 13.0)
                addSubview(button)
                table[key] = button
                y -= button.bounds.size.height + 20.0
                let imageFrame = NSRect(x: 8.0, y: y, width: frame.width, height: height)
                let flowView = F1FlowView(frame: imageFrame, flowDesc: key)
                addSubview(flowView)
                y -= flowView.frame.size.height - 16.0
            } else {
                let label = parameters.keysToUILabels[key]!
                let toolTip = parameters.toolTips[key] ?? ""
                let enumChoices = parameters.enumChoices
                let choices = enumChoices?[key]
                let pos: CGPoint = CGPoint(x: 10.0, y: y)
                let control = addLabelAndControl(pos, label: label, defaultValue: defaultValue, enumChoices: choices, toolTip: toolTip)
                y -= control.frame.size.height + 1.0
                if keyView != nil {
                    keyView.nextKeyView = control
                }
                keyView = control
                table[key] = control
            }
    ...
    func addLabelAndControl(_ pos: CGPoint, label: String, defaultValue: JSON, enumChoices: [String]!, toolTip: String) -> NSControl {
        enum ControlType {
            case textField
            case checkBox
            case popup
        ...
        var controlStart: CGFloat
        if type == .checkBox {
            fieldWidth = frame.width - 55
            controlStart = pos.x
        } else {
            // Add a text label, except in the checkBox case
            fieldWidth = type == .textField ? 58.0 : 138.0
            let labelWidth = frame.width - 13.0 - fieldWidth
            let labelFrame = NSRect(x: pos.x, y: pos.y - 6.0, width: labelWidth, height: 22)
            let textLabel = NSText(nonSelectableText: label + ":", frame: labelFrame)
            controlStart = labelWidth + 9.0 // leave a bit of extra space
        ...
        switch type {
            case .textField:
                control = NSTextField()
                control.stringValue = defaultValue.description
            case .checkBox:
                let button = NSButton()
                button.setButtonType(.`switch`)
                button.title = label + ":"
                button.imagePosition = .imageRight
                button.state = defaultValue.boolValue ? 1 : 0
                control = button
            case .popup:
                let choicesButton = NSPopUpButton()
                choicesButton.addItems(withTitles: enumChoices)
                choicesButton.selectItem(withTitle: defaultValue.stringValue)
                choicesButton.synchronizeTitleAndSelectedItem()
                control = choicesButton
        }
        control.alignment = NSTextAlignment.left
        control.frame = NSRect(x: controlStart, y: pos.y, width: fieldWidth, height: 22)
...
/*=============== BUILDING UI UTILITIES ===============*/

class MappingOfParametersToControls {
    var table: [String: [SimulationParameterKey: SimulationParameterControl]] = [:] // maps each parameters name to the keys and controls for that parameters
    func makeBoldForParameters(_ allParameters: FunChipSimulationParameters) -> Set<SimulationCategory> {
        // Bolds the controls that are non-default values
        // Also returns, for each key's category, which category should be bolded
        var bold: Set<SimulationCategory> = []
        for parameters in allParameters.allParametersWithUI {
            let name = parameters.name
            let t: [SimulationParameterKey: SimulationParameterControl]! = table[name]
            if t != nil {
                parameters.updateFromControls(t) {
                    let control = $1 as! NSControl
                    control.makeOrResetBold($2)
                    if $2 {
                        bold.insert(parameters.keyCategory($0))
                    }
                }
            }
        }
    ...
    func mergeInCategory(_ name: String, t: [SimulationParameterKey: SimulationParameterControl]) {
        var sub = table[name]
        if sub == nil { sub = [:] }
        sub!.unionInPlace(t)
        table[name] = sub
    }
    func reflectParametersIntoUI(_ parameters: SimulationParameters) {
        let name = parameters.name
        let t: [SimulationParameterKey: SimulationParameterControl]! = table[name]
        if t == nil { return }
        for tabID in SimulationParametersTabs {
            ...
            for key in keys {
                ...
                let value = parameters.all[key]
                if value == nil { continue }
                control!.setValueFromJSON(value!)
            }
        }
    }
    func reflectParametersJSONIntoUI(_ json: JSON) {
        let allParameters = FunChipSimulationParameters()
        allParameters.updateFromJSON(json)
        allParameters.allParametersWithUI.forEach(reflectParametersIntoUI)
    }
}
