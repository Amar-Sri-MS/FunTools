...
def get_host_pkt_info(rx):
        ...
        for lines in stdout.read().splitlines():
            if "HWaddr" in lines.decode("ascii"):
                mac = lines.decode('ascii').split()[-1]
...
def main():
        ...
        arg_parser.add_argument('--srv_intf', type=str, default="",help="TG interface towards DPU, to pump traffic")
        ...
        arg_parser.add_argument('--cli_intf', type=str, default="",help="DPU interface towards TG.")
        ...
        if(args.srv_user != ""):
            srv["user"]=args.srv_user
        if(args.srv_password != ""):
            srv["pass"]=args.srv_password
        if(args.srv_intf != ""):
            srv["intf"]=args.srv_intf

        if(args.cli_hostname != ""):
            cli["name"]=args.cli_hostname
        if(args.cli_user != ""):
            cli["user"]=args.cli_user
        if(args.cli_password != ""):
            cli["pass"]=args.cli_password
        if(args.cli_intf != ""):
            cli["intf"]=args.cli_intf
        ...
        dmac,dip = get_host_pkt_info(rx)
        smac,sip = get_qfx_pkt_info(tx)
        ...
        for pid in pid_list:
            cmd += " " + pid
        print("stop health monitor on DPU")
        stdin, stdout, stderr = rx.exec_command(cmd, get_pty=True)
