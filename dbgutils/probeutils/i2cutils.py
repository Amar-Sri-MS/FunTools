...
def _i2c_dbg_chal_read(dev, length):
    ...
    read_status_data = aa_i2c_read(dev, constants.F1_I2C_SLAVE_ADDR, 0, rdata)
    logger.debug('Read data: {0}'.format(rdata))
...
def i2c_wedge_detect(dev):
    logger.info('Checking i2c wedge condition ...!')
    data = array('B', [])
    (status, sent_bytes) = aa_i2c_write_ext(dev, constants.F1_I2C_SLAVE_ADDR, 0, data)
...
def i2c_unwedge(dev):
    ...
    status = aa_configure(dev, constants.IC_DEVICE_MODE_GPIO)
    status = aa_gpio_direction(dev, 0x1)
    for i in range(100):
        status = aa_gpio_set(dev, 0x1)
        time.sleep(0.0001)
        status = aa_gpio_set(dev, 0x00)
        time.sleep(0.0001)
        status = aa_gpio_set(dev, 0x1)
        time.sleep(0.0001)
    status = aa_configure(dev, constants.IC_DEVICE_MODE_I2C)
