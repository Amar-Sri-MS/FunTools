...
curl -X PUT "localhost:9200/_ilm/policy/fod" -H 'Content-Type: application/json' -d'
{
  "policy": {
    "phases": {
      "hot" : {
...
curl -X PUT "localhost:9200/log_fod-*/_settings" -H 'Content-Type: application/json' -d'
{
  "index": {
    "lifecycle": {
      "name": "fod"
