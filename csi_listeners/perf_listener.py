...
#
# Usage: perf_listener.py -h for help on usage
#
...
class PerfWriter(listener_lib.BaseWriter):
    ...
    def start_new_run(self, new_dir):
        """
        Tells the perf writer to restart its internal count
...
def main():
    ...
    parser.add_argument('--perf-ip', type=str,
                        help='IP address for FunOS traffic',
                        default='60.1.1.1')
    parser.add_argument('--perf-port', type=int,
    ...
    tcp_serv_socket.bind((args.perf_ip, args.perf_port))
    tcp_serv_socket.listen(1)
    log('Listening on IP %s and port %d' % (args.perf_ip, args.perf_port))

    buf = Buffer()
    rdsock_serv = RdsockServer(tcp_serv_socket, buf, RdsockHandler,
                               shutdown_threads)
    perf_writer = PerfWriter(buf, trace_dir)
