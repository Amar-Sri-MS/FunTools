...
class QosCommands(object):
    ...
    def egress_buffer_pool(self, sf_thr=None, sx_thr=None, df_thr=None, dx_thr=None, fcp_thr=None, nonfcp_thr=None,
                           sample_copy_thr=None, sf_xoff_thr=None, sf_xon_thr=None, fcp_xoff_thr=None, nonfcp_xoff_thr=None,
...
class PeekCommands(object):
        ...
        try:
            ...
            while True:
                try:
                    cmd = "stats/ddr"
                    result = self.dpc_client.execute(verb='peek', arg_list=[cmd])
                    mud0_txn = result["mud_0"]["cna_read_requests"] + result["mud_0"]["cna_write_requests"] \
                           + result["mud_0"]["dna_write_requests"] + result["mud_0"]["sna_read_requests"]
                    mud0_bw = mud0_txn * 64 * 8
                    mud1_txn = result["mud_1"]["cna_read_requests"] + result["mud_1"]["cna_write_requests"] \
                           + result["mud_1"]["dna_write_requests"] + result["mud_1"]["sna_read_requests"]
                    mud1_bw = mud1_txn * 64 * 8
                    total = {"total": {"ddr_transactions": mud0_txn + mud1_txn,\
    ...
    def peek_sdn_stats(self, offset, num_flows, grep=None):
        try:
            ...
            while True:
                try:
                    arg_dict = {"flow": "show", "flow_offset": offset, "num_flows": num_flows}
                    cmd = ["sdn", arg_dict] 
                    result = self.dpc_client.execute(verb='network', arg_list=cmd)
                    if result:
                        result["total_installed_flows"] = {"tot_installed_flows": result["installed_flows"]}
    ...
    def peek_sdn_meter_stats(self, policy_id, direction, grep=None):
        ...
        while True:
            try:
                ...
                result = self._get_sdn_meter_stats(policy_id=policy_id, direction=direction) 
                if prev_result:
                    table_obj = PrettyTable(['Name', 'Bucket Id', 'Bytes', 'Bytes diff','Pkts', 'Pkts diff'])
                    table_obj.align = 'l'
                    for entry in result:
                        bytes_diff = 0
                        pkts_diff = 0
                        for prev_entry in prev_result:
                            if prev_entry['bucket_id'] == entry['bucket_id']:
                                ...
                                break
                        table_obj.add_row([entry['name'], entry['bucket_id'], entry['bytes'], bytes_diff,
                                           entry['pkts'], pkts_diff])
                else:
                ...
                for flow_id in flow_ids:
                    ...
                    if 'layers' in flow_details:
                        ...
                        for layer in all_layers:
                            ...
                            table_obj.add_row([flow_id.split("_")[-1], forward_data['sip'], forward_data['sport'],
                                               forward_data['dip'], forward_data['dport'], layer['protocol'],
                                               layer['layer_id'], layer['action'], forward_action_data])
                    ...
                    if old_fwd_flows:
                        ...
                        for flow in sorted_flows:
                            ...
                            if flow not in old_fwd_flows:
                                ...
                                f_pkt_diff = fflow['stats_pkts'] - old_f_flow['stats_pkts']
                                f_byte_diff = fflow['stats_bytes'] - old_f_flow['stats_bytes']
                                r_pkt_diff = rflow['stats_pkts'] - old_r_flow['stats_pkts']
                                r_byte_diff = rflow['stats_bytes'] - old_r_flow['stats_bytes']

                            table_obj.add_row([flow_id + "_f", fflow['sip'], fflow['sport'],
                                               fflow['dip'], fflow['dport'],
                                               fflow['protocol'], fflow['stats_pkts'], f_pkt_diff,
                                               fflow['stats_bytes'], f_byte_diff])
                            table_obj.add_row(
                                [flow_id + "_r", rflow['sip'], rflow['sport'], rflow['dip'], rflow['dport'],
                                 rflow['protocol'], rflow['stats_pkts'],r_pkt_diff, rflow['stats_bytes'], r_byte_diff])
                    else:
                        table_obj = PrettyTable(['flow_id', 'sip', 'sport', 'dip', 'dport', 'protocol', 'pkt', 'byte'])
                        table_obj.align = 'l'
                        sorted_flows = list(fwd_flows.keys())
                        ...
                        for flow in sorted_flows:
                            ...
                            fflow = fwd_flows[flow]
                            rflow = rev_flows['r_' + flow]
                            table_obj.add_row([flow_id + "_f", fflow['sip'], fflow['sport'], fflow['dip'],
                                               fflow['dport'], fflow['protocol'], fflow['stats_pkts'],
                                               fflow['stats_bytes']])
                            table_obj.add_row([flow_id + "_r", rflow['sip'], rflow['sport'], rflow['dip'], rflow['dport'],
    ...
    def peek_tcp_flows_state(self,flow_id = -1,sip=None,dip=None,sport=None,dport=None,count=None):
        ...
        if arg_dict: cmd.append(arg_dict)
        try:
            result = self.dpc_client.execute(verb="tcp",arg_list=cmd)
            if result:
                tcp_dict = result['6.0.0']['tcp']
        ...
        try:
            ...
            if result:
                ...
                for _entry in tcp_dict:
                    ...
                    table_obj = PrettyTable([_entry['flow']['id'],'res_hu_pc','res_hu_r_pc','res_nu','res_nu_r_pc'])
                    table_obj.align = 'l' 
                    bam_entry = [tcp_entry['hu_or_pc_res_stats']['res_fc_cnt']['BAM DEFAULT ALLOC POOL'],tcp_entry['hu_or_pc_res_stats']['res_r_fc_cnt']['BAM DEFAULT ALLOC POOL'],tcp_entry['nu_res_stats']['res_fc_cnt']['BAM ETP CMDLIST'],tcp_entry['nu_res_stats']['res_r_fc_cnt']['BAM ETP CMDLIST']]
                    dma_entry = [tcp_entry['hu_or_pc_res_stats']['res_fc_cnt']['HU OR PC DMA QUEUE'],tcp_entry['hu_or_pc_res_stats']['res_r_fc_cnt']['HU OR PC DMA QUEUE'],tcp_entry['nu_res_stats']['res_fc_cnt']['NU ETP DMA QUEUE'],tcp_entry['nu_res_stats']['res_r_fc_cnt']['NU ETP DMA QUEUE']]
                    wu_entry = [tcp_entry['hu_or_pc_res_stats']['res_fc_cnt']['HU TILE WU COUNT'],tcp_entry['hu_or_pc_res_stats']['res_r_fc_cnt']['HU TILE WU COUNT'],tcp_entry['nu_res_stats']['res_fc_cnt']['NU WU COUNT'],tcp_entry['nu_res_stats']['res_r_fc_cnt']['NU WU COUNT']]
                    blocked_entry = [tcp_entry['hu_or_pc_res_stats']['blocked_cnt'],tcp_entry['nu_res_stats']['blocked_cnt']]
                    restart_entry = [tcp_entry['hu_or_pc_res_stats']['restart_cnt'],tcp_entry['nu_res_stats']['restart_cnt']]
                    buff_entry = tcp_entry['xmt_res_page_buf_cnt']
                    buff_r_entry = tcp_entry['xmt_res_page_buf_restart_cnt']
                    if any(bam_entry) or any(dma_entry) or any(wu_entry) or any(blocked_entry) or any(restart_entry) or buff_entry or buff_r_entry:
                        table_obj.add_row(['BAM'] + bam_entry)
                        table_obj.add_row(['DMA Q'] + dma_entry)
                        table_obj.add_row(['WU Count'] + wu_entry)
                        table_obj.add_row(['Blocked',tcp_entry['hu_or_pc_res_stats']['blocked_cnt'],'X',tcp_entry['nu_res_stats']['blocked_cnt'],'X'])
                        table_obj.add_row(['Restart',tcp_entry['hu_or_pc_res_stats']['restart_cnt'],'X',tcp_entry['nu_res_stats']['restart_cnt'],'X'])
    ...
    def peek_tcp_flows_stats(self,flow_id = -1,sip=None,dip=None,sport=None,dport=None,count=None,drops=False,rate=False,other=False, iterations=9999999):
        ...
        prev_result = {}
        key_len = 0
        if rate: key_len = len("total_")
        elif drops:key_len = len("drops_")
        else: key_len = len("num_")
        while True:
            try:
                result = self.dpc_client.execute(verb="tcp",arg_list=cmd)
                if '6.0.0' in result:
                    ...
                    for key in sorted(sample_dict):
                        if (drops and "drop" in str(key)) or (rate and "total_" in str(key)) or (other and "num_" in str(key)):
                    ...
                    for entry in tcp_dict:
                        ...
                        if entry['flow']['id'] not in prev_result:
                            ...
                            for key in stat_dict:
                                row_entry.append(str(stat_dict[key])+ " | " +  str(diff_result[key]))
    ...
    def peek_stats_probetest(self,summary=None,localityinfo=None,pathinfo=None,path_id=-1,status=None,tunnelinfo=None,streaminfo=None):
        ...
        try:
            if summary:
                ...
                arg_list = []
                if status is not None:
                    if status == 'all': arg_list.append('all')
                    elif status == 'up': arg_list.extend(['all','up'])
                    elif status == 'down': arg_list.extend(['all','down'])
                else:
                    arg_list.append("pathinfo")
                result = self.dpc_client.execute(verb="probetest",arg_list=arg_list)
                if status is not None: result = result['pathinfo']
                if result:
                    encap_map = { 0:'NONENCAP',3:'IPINIP',4:'GRE'}
                    state_map = { 0:'UNINIT', 1:'WAIT_FOR_REPLY', 2:'ADMIN_DOWN', 3:'RUNNING'}
                    status_map = {0:'DOWN',1:'UP'}
                    locality_map =  { 0:'WITHINTOR',2:'INTRATOR',3:'INTRAPOD'}
                    key_filter = ['cfg_encap_type','cfg_locality','cfg_outgoing_port','cfg_sport','count_rcv','count_retry','count_send','debug_rtt_rcv_1','debug_rtt_rcv_2','debug_rtt_total','flag_last_update','state','status']
                    table_obj = PrettyTable(['Path_id'] + ['encap_type','locality','outgoing_port','sport','rcv','retry','send','rtt_rcv_1','rtt_rcv_2','rtt_total','last_update','state','status'])
                    path_cnt = 0
                    for path in result:
                        path_dict = dict()
                        ...
                        path_dict['cfg_encap_type'] = encap_map[path_dict['cfg_encap_type']]
    ...
    def peek_fcp_path_stats(self,path_id = -1):
        ...
        while True:
            try:
                result = self.dpc_client.execute(verb="peek",arg_list=['stats/fcp_paths'])
                if result:
                    table_obj = PrettyTable(['Path ID','Rcv count','Retry count','Rtt','Send count','State','Status'])
                    row_cnt = 0
                    if prev_result:
                        table_obj = PrettyTable(['Path ID','Rcv count','Rcv Diff','Retry count','Retry Diff','Rtt','Rtt Diff','Send count','Send Diff','State','State change','Status','Status Change'])
                    for path in result:
                        path_dict = dict()
                    ...
                    for path,tunnel in (list(result.items())):
                        ...
                        for entry in tunnel:
                            ...
                            if idx not in prev_result:
                                ...
                                diff_result = self._get_difference(result=tunnel_dict, prev_result=prev_result[idx])
                                row_entry = [ftep]
                                for key in tunnel_dict:
                                    row_entry.extend([tunnel_dict[key],diff_result[key]])
                                table_obj.add_row(row_entry)
                                row_cnt += 1
            ...
            for core in output_result[cluster]:
                ...
                if vp_len != 4:
                    for i in range(vp_len, 4):
                        key = "vp_"+str(i)
                        val = {"q_high_depth": 0, "q_low_depth": 0, "wus_received": 0, "wus_sent": 0, "wus_high_received": 0, "wus_high_sent": 0}
                        ...
                        def get_complete_dict(result, tabular_list, prev_result=None):
                            ...
                            for cs_id in cluster_ids:
                                ...
                                for c_id in core_ids:
                                    ...
                                    clus_core_val = "%s/%s" % (current_cs_id, current_c_id)
                                    ...
                                    for v_id in vp_ids:
                                        ...
                                        col_start = get_list_index(current_v_id, tabular_list)
                                        col_end = get_list_index(current_v_id, tabular_list, True)
                                        for item in tabular_list[col_start:col_end]:
                                            if (not 'd_' in item) and item.split(":")[0] == current_v_id:
                                                vp_key_name = item.split(":")[1]
                                                cur_val = current_result[cs_id][c_id][v_id].get(vp_key_name, 0)
                                ...
                                for cs_id in cluster_ids:
                                    ...
                                    for c_id in core_ids:
                                        ...
                                        for v_id in vp_ids:
                                            ...
                                            col_end = get_list_index(current_v_id, tabular_list, True)
                                            for item in tabular_list[col_start:col_end]:
                                                if ('d_' in item) and item.split(":")[0] == current_v_id:
                                                    vp_key_name = item.split(":")[1].split("_",1)[1]
                                                    cur_val = diff_result[cs_id][c_id][v_id].get(vp_key_name, 0)
                                                    complete_dict[item].append(cur_val)
                            ...
                            for cs_id in cluster_ids:
                                ...
                                for c_id in core_ids:
                                    ...
                                    for v_id in vp_ids:
                                        ...
                                        col_end = get_list_index(current_v_id, tabular_list, True)
                                        for item in tabular_list[col_start:col_end]:
                                            if item.split(":")[0] == current_v_id and not item.startswith(str(current_v_id) + ':d_'):
                                                vp_key_name = item.split(":")[1]
                                                cur_val = current_result[cs_id][c_id][v_id].get(vp_key_name, 0)
                                                complete_dict[item].append(cur_val)
                                ...
                                for cs_id in cluster_ids:
                                    ...
                                    for c_id in core_ids:
                                        ...
                                        for v_id in vp_ids:
                                            ...
                                            col_end = get_list_index(current_v_id, tabular_list, True)
                                            for item in tabular_list[col_start:col_end]:
                                                if item.split(":")[0] == current_v_id and item.startswith(str(current_v_id) + ':d_'):
                                                    vp_key_name = item.split(":")[1].split("_",1)[1]
                                                    cur_val = diff_result[cs_id][c_id][v_id].get(vp_key_name, 0)
                                                    complete_dict[item].append(cur_val)
    ...
    def peek_bam_resource_stats(self, cid=None, diff=None, grep_regex=None, get_result_only=False, iterations=9999999):
        ...
        while True:
            try:
                ...
                if not result:
                    break
                reference_keys = self._get_max_reference_keys(result['bm_usage_per_pool_per_cluster'],
