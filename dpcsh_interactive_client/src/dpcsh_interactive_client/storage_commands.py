...
class StorageCommands(object):
        ...
        try:
            ...
            if result:
                table_obj = PrettyTable(
                    ["sl no", "volume uuid", "pvg_uuid", "lsv_uuid"]
                )
                table_obj.align = "l"
                counter = 0
                for key, val in result.items():
                    ...
                    if result1:
                        for key, val in result1.items():
                            for ikey in val:
                                if ikey == "md_vol_uuid":
                                    table_obj.add_row(
                                        [counter, pv_uuid, pvg_uuid, val[ikey]]
                                    )
                print("******* %s ********" % voltype)
                print(table_obj)
        except Exception as ex:
    ...
    def storage_list_volumes_blt(self, voltype):
        ...
        try:
            ...
            if result:
                table_obj = PrettyTable(
                    ["sl no", "volume uuid", "drive_uuid", "drive_id"]
                )
                table_obj.align = "l"
                counter = 0
                for key, val in result.items():
                ...
                for key, val in result.items():
                    ...
                    for ikey, ival in val.items():
                        for inner_key in ival:
                            if (
                                inner_key == "remote_ip"
                                or inner_key == "remote_nsid"
                                or inner_key == "remote_port"
                                or inner_key == "remote_vol_type"
                                or inner_key == "subsys_nqn"
                                or inner_key == "connections"
                                or inner_key == "md_size"
                                or inner_key == "ctrlr_id"
                            ):
                                info_obj = (
                                    info_obj
                                    + str(inner_key)
    ...
    def storage_list_volumes_replica(self, voltype, jvol_uuid=None, svol_uuid=None):
        ...
        try:
            ...
            if result:
                table_obj = PrettyTable(
                    ["sl no", "volume uuid", "nplexes", "pvol-uuids"]
                )
                table_obj.align = "l"
                counter = 0
                num_replicas = None
    ...
    def storage_list_volumes_lsv(self, voltype):
        ...
        try:
            ...
            if result:
                table_obj = PrettyTable(
                    ["sl no", "volume uuid", "svol_uuid", "jvol_uuid"]
                )
                table_obj.align = "l"
                counter = 0
                for key, val in result.items():
    ...
    def storage_getmap_volume(
        self, vol_uuid, slba, lbc, volume="volume", opcode="GET_MAP"
    ):
        try:
        ...
        try:
            ...
            while True:
                try:
                    ...
                    if result:
                        if prev_result:
                            diff_result = storagepeekcommands._get_difference(
    ...
    def storage_rdsock_vp_stats(
        self, controller="controller", opcode="GET_RDSOCK_VP_STATS"
    ):
        try:
                            ...
                            for inner_key in result[key][_key]:
                                if inner_key in prev_result[key][_key]:
                                    if type(result[key][_key][inner_key]) == str:
                                        diff_result[key][_key][inner_key] = result[key][
                                            _key
                                        ][inner_key]
                                    else:
                                        diff_value = (
            ...
            if result:
                for key, val in result.items():
                    ...
                    if key == VOL_TYPE_BLK_LOCAL_THIN:
                        ...
                        table_obj = PrettyTable(
                            ["sl no", "volume_uuid", "drive_uuid", "drive_id"]
                        )
                        table_obj.align = "l"
                        counter = 0
                        print_table = False
