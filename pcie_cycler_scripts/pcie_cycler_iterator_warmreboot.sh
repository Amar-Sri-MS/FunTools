...
do
   ...
   then
	...
	do
		echo $server
		logfilename="~/pcie_cycler/log_warmreboot_"$server"_"$sshcount"_"$currdate
		output=`sshpass -p "Precious1*" ssh localadmin@$server "~/pcie_cycler/pcie_cycler_warmreboot.sh $logfilename"`
		ret=$?
   		if [ $ret == 0 ]
		then
			sshcount=$(( $sshcount + 1 ))
